...
orthofunc<-function(i) {
  ortho.lm<-lm(hadcrut$V2~poly(hadcrut$time.date,i,raw=FALSE) + hadcrut$monthf,data=hadcrut)
  return (summary(ortho.lm))
}
orthofunc(10) #"large p value"
final.lm<-lm(hadcrut$V2~poly(hadcrut$time.date,9,raw=FALSE) + hadcrut$monthf,data=hadcrut)
