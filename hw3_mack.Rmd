...
Assessing the plot of the ensemble medians versus time, we can see an overall positive trend since January of 1850. The trend has curvature, suggesting a cyclic trend in ensemble medians throughout the timeline. It will be interesting to see if we are currently at a peak in ensemble medians.The resolution of the plot below does not allow us to visally assess seasonality trends. The variability  appears to be larger in the first 500 years of the data set as we can see a range of observations  between about -1.5 and 0.7. In the later years, this spread appears to have decreased. Across the entire 156 year interval, there are a few observations that stand out from the rest of the data as outliers, especially in the first five hundred years. 

The last two plots are zoomed in to the first four years. There appears to be seasonality, but the seasonality does only appears to be somewhat consistent across the first four years during the first six months of the years.
...
```{r prob3x, fig.height = 6, fig.width = 8}
...
plot(hadcrut[12:24,]$V2 ~ hadcrut[12:24,]$date, xlab = "Months Since January 1850", ylab = "Ensemble Medians")
par(mfrow = c(2,1))
plot(hadcrut[24:36,]$V2 ~ hadcrut[24:36,]$date, xlab = "Months Since January 1850", ylab = "Ensemble Medians")
plot(hadcrut[36:48,]$V2 ~ hadcrut[36:48,]$date, xlab = "Months Since January 1850", ylab = "Ensemble Medians")
